...
def tune(test_nn, nn_train_epochs, skip_epoch, llm_path, llm_tune_conf, nn_gen_conf, conf_keys, llm_conf, training_args,
    ...
    for epoch in range(llm_tune_epochs):
        ...
        if epoch < skip_epoch:
            ...
            if trans_mode:
                trans_gen(epoch, out_path, chat_bot, conf_keys, nn_train_epochs, prompt_dict, test_nn, max_new_tokens,
        ...
        if trans_mode:
            ...
            if not use_unsloth:
                data_processor = NNGenPrompt(context_length if context_length else model_loader.get_max_length(),
                                             tokenizer, train_config_path)
            else:
                data_processor = NNGenPrompt(
                    unsloth_max_input_length if unsloth_max_input_length else model_loader.get_max_length(), tokenizer,
                    train_config_path)
...
def nn_gen(epoch, out_path, chat_bot, conf_keys, nn_train_epochs, prompt_dict, test_nn, max_new_tokens, save_llm_output,
    ...
    for idx, prompt in tqdm(enumerate(prompts)):
        ...
        if use_delta and origdf is not None:
            try:
                    ...
                    if not validate_delta(delta):
                        ...
                        if baseline_code:
                            ...
                            if applied_code:
                                ...
                                print(
                                    f'[WARNING] Failed to apply delta for model B{idx} (delta application returned None), using extracted code as fallback')
                                # code already extracted above, keep it
                        else:
                ...
                print(
                    f'[ERROR] Failed to import delta utilities for model B{idx}: {e}. Using extracted code as fallback.')
            except Exception as e:
                print(
                    f'[WARNING] Unexpected error applying delta for model B{idx}: {e}. Using extracted code as fallback.')
                # code already extracted above, keep it
